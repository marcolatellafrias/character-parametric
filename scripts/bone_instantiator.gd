...
func initialize_skeleton() -> void:
	...
	ik_util = IkUtil.create(skel_sizes_util, custom_bones_util)
	#Luego creo el character rigidbody
	#var camera3d = Camera3D.new()
	#camera3d.position = Vector3(0, 1.7, 2.7)
	char_rigidbody = CharacterRigidBody3D.create(skel_sizes_util.lower_spine_size,ik_util.left_leg_raycast,ik_util.right_leg_raycast)
