...
var global_targets:   Node3D
var local_targets:    Node3D
var skel_rigidbodies: Node3D
var joints:           Node3D
...
func _ready() -> void:
	for node_name in ["global_targets", "local_targets", "skel_rigidbodies", "joints"]:
		if not has_node(node_name):
			var n := Node3D.new()
			n.name = node_name
			add_child(n)
	global_targets   = get_node("global_targets")
	local_targets    = get_node("local_targets")
	skel_rigidbodies = get_node("skel_rigidbodies")
	joints           = get_node("joints")

	# PlayerController solo para el jugador local
	if is_active:
...
func initialize_skeleton() -> void:
	...
	# Peers no-authority no simulan física, solo reciben posición vía RPC
	...
	if is_instance_valid(player_controller):
		if is_active:
			...
			set_first_person_visibility(true)
		else:
			set_first_person_visibility(false)
...
# --- Sincronización de posición por RPC (reemplaza MultiplayerSynchronizer) ---

@rpc("authority", "unreliable_ordered", "call_remote")
func _rpc_sync_pos(pos: Vector3, rot_y: float) -> void:
	if not is_instance_valid(char_rigidbody):
		return
	char_rigidbody.global_position = pos
	char_rigidbody.rotation.y      = rot_y
...
func _physics_process(delta: float) -> void:
	...
	# Authority: broadcast posición a los demás peers
	if multiplayer.multiplayer_peer != null and is_multiplayer_authority():
		_rpc_sync_pos.rpc(char_rigidbody.global_position, char_rigidbody.rotation.y)
