...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		if _is_rotating and is_instance_valid(_grabbed):
			_grabbed.rotate(camera.global_transform.basis.x, -event.relative.y * grab_rotation_sensitivity)
			_grabbed.rotate(camera.global_transform.basis.y, -event.relative.x * grab_rotation_sensitivity)
		else:
...
func _update_curve() -> void:
	...
	var points := [
		{ "pos": _get_grab_origin(), "in": Vector3.ZERO, "out": Vector3.ZERO },
		{ "pos": _grabbed.global_position, "in": Vector3.ZERO, "out": Vector3.ZERO }
	]

	_curve_mesh = DebugUtil.create_debug_path3d(points, 1, Color(0.4, 0.9, 1.0), 0.005)
