...
@export var throw_arm_charge_factor: float = 0.5
@export var throw_arm_push_factor:   float = 1.5
...
func setup(anim_modifiers: AnimationModifiers) -> void:
	anim_mod = anim_modifiers

func _physics_process(delta: float) -> void:
	if not is_instance_valid(bi):
		return
	_update_arm_compress()
	_update_throw_arms(delta)
	_update_grab_arms()

func _update_arm_compress() -> void:
	var rb  := bi.char_rigidbody
	var arm_bentness := bi.entity_instantiation.arch_final.arm_bentness
	var remaining    := 1.0 - arm_bentness
	var raw: float = bi.jump_squat_t * ARM_COMPRESS_JUMP \
		+ bi.crouch_t * ARM_COMPRESS_CROUCH \
		+ max(0.0, rb.impact_y) * ARM_COMPRESS_IMPACT
	bi.locomotion_signals.arm_compress = clamp(raw * remaining, 0.0, remaining)

func _update_throw_arms(delta: float) -> void:
	...
	if throw_push_t > 0.0:
		...
		var side_ext       := arm_total * throw_arm_charge_factor * lerpf(1.0, 0.4, abs(pitch_norm))
		...
		var elbow_down     : Vector3 = -char_up * arm_total * (0.25 + pitch_norm * 0.2)
...
func _sync_throw_locals_to_current() -> void:
	...
	var cb    := bi.custom_bones_util
	var rb    := bi.char_rigidbody
	var inv   := rb.global_transform.basis.inverse()
