...
var _last_impact_height: float = 0.0
...
func update_impact_debug(
	...
	_last_impact_height = p_last_impact_height
...
func _draw() -> void:
	...
	_height_bar(Vector2(cx2 + CR + PAD + 2.0, cy2), _last_impact_height)
	var ic := C_FG if absf(_last_impact_height) < 0.5 else (C_WARN if absf(_last_impact_height) < 0.8 else C_ALERT)
	draw_string(f, Vector2(cx2 + CR + PAD + 18.0, cy2 - 4.0), "mag %.2f" % _last_impact_magnitude,
		HORIZONTAL_ALIGNMENT_LEFT, -1, FS, C_FG)
	draw_string(f, Vector2(cx2 + CR + PAD + 18.0, cy2 + 10.0), "h   %.2f" % _last_impact_height,
		HORIZONTAL_ALIGNMENT_LEFT, -1, FS, ic)
	y += CR * 2.0 + 6.0; _sep(y); y += 8.0
...
func _height_bar(center: Vector2, h: float) -> void:
	var bh := CR * 1.8
	draw_line(center + Vector2(0.0, -bh * 0.5), center + Vector2(0.0, bh * 0.5), C_DIM, OW)
	draw_line(center + Vector2(-3.0, 0.0), center + Vector2(3.0, 0.0), C_DIM, OW)
	draw_line(center + Vector2(-2.0, -bh * 0.5), center + Vector2(2.0, -bh * 0.5), C_DIM, OW)
	draw_line(center + Vector2(-2.0,  bh * 0.5), center + Vector2(2.0,  bh * 0.5), C_DIM, OW)
	var dy : float = center.y - clamp(h, -1.0, 1.0) * bh * 0.5
	var hc := C_FG if absf(h) < 0.5 else (C_WARN if absf(h) < 0.8 else C_ALERT)
	draw_circle(Vector2(center.x, dy), 3.0, hc)
