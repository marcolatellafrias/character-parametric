...
func _apply_movement_force() -> void:
	...
	var is_sprinting := Input.is_action_pressed("sprint") # reemplaza con tu action de shift
	var current_max_speed := max_move_speed * (sprint_multiplier if is_sprinting else 1.0)
	var current_force := move_force * (sprint_multiplier if is_sprinting else 1.0)
	
	var brake: Vector3 = -horizontal_vel.normalized() * min(horizontal_vel.length(), current_max_speed)
