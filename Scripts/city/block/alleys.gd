...
func _expand_side(
	...
	while not _is_vertex_on_boundary(current, target_boundary):
		...
		if result["vertex"] == Vector2i(-1, -1):
			print("  Sin más opciones válidas")
			break
		...
		print("  %s → %s (%s, steps=%d)" % [current, next_v, 
			"TURN" if is_direction_change else "STRAIGHT", 
		...
		if is_merge:
			print("  ✓ MERGE con path existente del mismo tipo en %s" % next_v)
			break
	...
	if _is_vertex_on_boundary(current, target_boundary):
		print("  Llegó a boundary %s" % target_boundary)
