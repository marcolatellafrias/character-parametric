...
func _build_graph_from_delaunay(
    ...
    # NORMALIZAR todas las caras para que tengan el mismo orden
...
func _normalize_face_orientation(faces_array: Array, desired_clockwise: bool = true) -> void:
    for i in range(faces_array.size()):
        ...
        var is_clockwise = _is_face_clockwise_3d(face)
...
func _is_face_clockwise_3d(face: Array) -> bool:
    var face_2d: Array[Vector2] = []
    
    for idx in face:
        var p = points[idx]
        face_2d.append(Vector2(p.x, p.z))
    
    var area = 0.0
    for i in range(face_2d.size()):
        var v1 = face_2d[i]
        var v2 = face_2d[(i + 1) % face_2d.size()]
