...
func _assign_traffic_light_indices() -> void:
    print("[GraphCityGenerator] Asignando índices de semáforos...")
    
    var total_assigned = 0
    
    for node_idx in range(plain_graph.points.size()):
        _assign_indices_for_node(node_idx)
...
func _assign_indices_for_node(node_idx: int) -> void:
    ...
    for key in lane_volume_areas:
        var lane_volume: LaneVolume = lane_volume_areas[key]
        var end_node = _get_lane_volume_end_node(lane_volume)
        
        if end_node == node_idx:
            volumes_at_node.append(lane_volume)
    
    if volumes_at_node.is_empty():
        return
    
    var available = volumes_at_node.duplicate()
    var current_index = 0
...
func _get_lane_volume_end_node(lane_volume: LaneVolume) -> int:
    var face_idx = lane_volume.face_idx
    var edge_idx = lane_volume.edge_idx
    var face = plain_graph.faces[face_idx]
    
    var node1_idx = face[edge_idx]
    var node2_idx = face[(edge_idx + 1) % face.size()]
    
    var center = lane_volume.get_center()
    var flow_dir = lane_volume.get_flow_direction()
    
    var node1_pos = plain_graph.points[node1_idx]
    var node2_pos = plain_graph.points[node2_idx]
    
    var to_node1 = (node1_pos - center).normalized()
    var to_node2 = (node2_pos - center).normalized()
    
    var dot1 = flow_dir.dot(to_node1)
    var dot2 = flow_dir.dot(to_node2)
    
    return node1_idx if dot1 > dot2 else node2_idx

func _find_best_opposite_pair(available_volumes: Array, node_idx: int) -> Array:
    ...
    for i in range(available_volumes.size()):
        for j in range(i + 1, available_volumes.size()):
            var vol1: LaneVolume = available_volumes[i]
            var vol2: LaneVolume = available_volumes[j]
            
            var dir1 = vol1.get_flow_direction()
            var dir2 = vol2.get_flow_direction()
