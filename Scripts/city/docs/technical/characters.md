...
Only a `ragdoll` **flag**, the **capsule (= pelvis) position**, and the **pelvis rotation** (`ragdoll_rot`, a quaternion, in the per-tick state) travel — never per-bone state. On the flag edge each proxy runs its **own local active ragdoll** through a single entry point — `RagdollUtil.activate_as_proxy` (and `deactivate_as_proxy` on exit), called from `CharacterNetSync._drive_proxy_ragdoll`. The model:

- **The pelvis (root) is kinematic and network-driven; every other bone simulates locally.** Each frame while the proxy ragdoll is active, `drive_pelvis_to(pos, rot)` sets the lower-spine body's transform **directly** to the synced pose (`pos` = `s["pos"]`, `rot` = `s["ragdoll_rot"]`, absolute); `set_pelvis_kinematic(true)` makes it a frozen **kinematic anchor**. Because it moves smoothly (interpolated), the joints **drag the hips/limbs along** without being slammed, and the limbs flail on their own gravity+springs. So the ragdoll is in the **right place** (no drift, authoritative) with the bones **in their sockets** — the pose is an approximation, the position is exact.
- **Why this model:** driving the pelvis *dynamically* (velocity or force) makes it fight its own joints and separate the seams; teleporting *every* body spazzes the solver; a *pure* local sim looks right but drifts. A **kinematic root + local limbs** avoids all three.
- **Seed:** on activation the proxy seeds all bodies with the owner's `puppet_velocity` (the fall momentum, still held from just before the ragdoll) so the limbs start with the right energy.

**The onset is seamless because `activate_as_proxy` does two things, in order, before handing control to the per-frame drive:**

1. **Orient to the authoritative facing.** The bodies are built from the local skeleton, whose facing is the capsule's `rotation.y` — but on the ragdoll edge that yaw isn't a usable source: the owner stops writing `camera_yaw` and streams `rb.rotation.y ≈ 0` while ragdolling, so the proxy capsule interpolates toward 0 exactly as it activates. So the facing comes from the **authoritative** `ragdoll_rot`: `activate_as_proxy` writes `char_rb.rotation.y = Basis(pelvis_world_rot).get_euler().y` first. The joints are then built already on the owner's heading, and `drive_pelvis_to` drives the absolute rotation with no offset — the pelvis doesn't snap in yaw at onset.
2. **Build the joints from the current pose.** `_build_joints()` runs `sync_to_bones()` as its first line, so the bodies match the skeleton at the instant each joint pins its bone pair. On a proxy (half-rate solve → bodies a frame or two behind, see [character-animation.md](character-animation.md)) this is what keeps the arms/feet in their sockets rather than pinned offset from them.
