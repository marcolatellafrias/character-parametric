...
> ### ⚠️ Orient the proxy skeleton before activating
> `activate()` builds the ragdoll from the **local** skeleton pose — the bones hang off the capsule, so their facing is the capsule's `rotation.y`. But **the capsule yaw is unreliable on the ragdoll edge**: the owner stops writing `camera_yaw` and sends `rb.rotation.y ≈ 0` while ragdolling, so the proxy's capsule interpolates toward 0 right as it activates. Build from that and the ragdoll faces **north**, then the kinematic pelvis (driven to the authoritative `ragdoll_rot`) **snaps in yaw** — a "twist as soon as it ragdolls," direction-dependent (none at yaw 0, full at other headings). Fix: **before `activate()`, orient the proxy capsule to the authoritative facing** — `rb.rotation.y = Basis(s["ragdoll_rot"]).get_euler().y` (`_drive_proxy_ragdoll`). Now the joints are built already facing the owner's direction and `drive_pelvis_to` can drive the absolute rotation with no offset/blend. (The earlier "capture a start-offset and sync the delta" approach was a band-aid over this — it re-anchored the twist instead of removing it.)
