...
The ground is two meshes under one collider (`City._visualize_ground`): the **blocks** (every distorted-grid cell, `ground_color`) and the **streets** (`street_color`), one mesh for the whole city — a few thousand triangles, so no reason to split it — with **UVs in metres** (`u` across from one kerb, `v` along from the node) so an asphalt shader can run continuous along a street without seams per block. A street is the quad between the two blocks' kerb corners at each node (`temporal_lane_points`); an **intersection** is the polygon those kerb corners leave around a node — the gap every crossing used to have — drawn as a fan from its centre at the node's height, planar UVs. An edge with a single face and a real street type (the sandbox's block sample) gets a **half street**, kerb to graph edge; the city's own boundary edges are `BOUNDARY` and get nothing, as before. Lane volumes follow the same heights (see [traffic.md](traffic.md#lane-volumes)).
...
| **Free placement** (`FreePlacement`, [free_placement.gd](../../block/free_placement.gd)) | a quad plus a list of footprints with margins — not a grid | parked cars; next: containers, chimneys, the roof tank | never — an entity with a transform |
