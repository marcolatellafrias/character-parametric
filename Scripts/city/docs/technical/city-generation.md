...
**Clouds** come from the **Sunshine Clouds 2** addon (`addons/SunshineClouds2/`, MIT): volumetric, rendered as a `CompositorEffect`. In `Demo.tscn` the `WorldEnvironment` carries a `Compositor` with the clouds resource, and the `Clouds` node (`SunshineCloudsDriverGD`) animates the wind and tracks the sun (`world/DirectionalLight3D`), so when `CityFog` moves the sun for a preset the clouds relight. ⚠ The resource must be saved **in the Compositor** too: the driver's setter only adds it there when the node is already in the tree, which never happens while a scene loads. Needs Forward+ (compute shaders); invisible in headless. `WeatherPresets` does not drive them yet.
...
⚠ **The addon's defaults are planet-scale**: noise patterns of 300, 85 and 20 km, a layer between 1.5 and 15 km, ray steps of 100–500 m. From a 1.8 km city under heavy fog, the whole visible sky falls inside a single sample of each pattern, so it is either one uniform veil or nothing — never separate clouds. The scene's resource is rescaled to the city: layer at 350–1 100 m (above the towers and the ship), patterns of 12 km / 4 km / 1.5 km / 500 m, steps of 15–40 m, wind speeds scaled down with them, and less atmospheric haze.
