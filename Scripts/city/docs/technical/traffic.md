...
**Other static obstacles, later.** The planner's design is sound and stays as it is: static obstacles are *known before a route exists*, so a car's vertical profile is planned **once per route** against their boxes — no reactive probing, no claims. The way to add a new kind of obstacle that reaches into a street — a sign, a roof tank near a kerb, a cupola, a chimney — is not a new avoidance mechanism but a new **provider of blocked boxes** that returns the geometry *through the same function the object is drawn with* (as `span_faces` does for bridges), gathered by `_collect_geos` alongside the bridges. Whatever the renderer puts in the street is what the cars dodge, by construction. Merging static and moving obstacles into one reactive system (everything as registry claims) would be a large change and is not planned.
