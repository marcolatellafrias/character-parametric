...
Remote players used to freeze in their last standing pose when they fell. **Fix:** `CharacterNetSync` syncs a `ragdoll` flag in the per-tick state, and on the edge each proxy runs its **own local ragdoll** (`_drive_proxy_ragdoll` → `RagdollUtil.activate/deactivate`). It's **not re-simulated to match** (physics isn't deterministic across machines) — it's an *approximation*: a limp body near the right place, starting from the last synced pose/position. While the proxy ragdoll is **active**, the local sim drives its capsule (the synced transform is not applied); on **recovery** the synced transform resumes (so it stands back up at the owner's actual position). After `deactivate` the puppet state is restored (`setup_as_puppet` — `deactivate` un-freezes the capsule). This is the visible payoff of Cause A + B: shove a teammate → they fall → everyone sees it. *Known approximation:* the proxy ragdoll drifts from the owner's (independent local physics) and starts at rest (impact momentum isn't synced), so a long ragdoll can end somewhere slightly different and snap toward the owner's position on recovery — fine for chill co-op.
