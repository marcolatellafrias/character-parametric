...
Remote players used to freeze in their last standing pose when they fell. **Fix:** `CharacterNetSync` syncs a `ragdoll` flag in the per-tick state, and on the edge each proxy runs its **own local ragdoll** (`_drive_proxy_ragdoll` → `RagdollUtil.activate/deactivate`). The **pose** isn't re-simulated to match (physics isn't deterministic across machines) — it's an *approximation*: a limp body flailing believably. But the **global position is anchored to the owner's**: while the proxy ragdoll is active, `apply_to_puppet` calls `RagdollUtil.anchor_pelvis_to(s["pos"])`, which rigid-translates the whole proxy ragdoll so its pelvis matches the owner's streamed pelvis position (the owner's `_update_active` keeps its capsule = pelvis, so `s["pos"]` is the pelvis). So the *pose* is local, the *place* is authoritative — no drift, however far the player ragdolls. On **recovery** the normal synced transform resumes. After `deactivate` the puppet state is restored (`setup_as_puppet` — `deactivate` un-freezes the capsule). This is the visible payoff of Cause A + B: shove a teammate → they fall → everyone sees it, in the right spot. *Known approximation:* only the pose differs across machines (the limbs land slightly differently); the body's location matches.
