...
A **flying ship** the players use to move packages quickly across the city. Hulls are being tried side by side — a **glass dome** and a **box**, large and small — around the same interior. Components:
...
**The box** is a room of opaque blocks in two sizes: the **large** one, 252 × 126 × 252 cells (10.08 × 5.04 × 10.08 m) — the dome's footprint and height —, and the **small** one, half as wide at the same height: a 126-cell (5.04 m) cube. Both have the original cubic design's front window and back door, which are human-scale (scaled up with the room, the window would sit above the pilot's eyes). Its walls, ceiling and door are drawn half-transparent by default, so the crew can be watched from outside (**Acciones → Nave: paredes traslúcidas**). Each face is its own object: Godot sorts transparency per object, so a single-mesh dome would draw back faces over front ones from inside. The faces and the door follow from the segments, so they don't land on whole cells.

The dashboards form a **partial ring against the hull**, facing the centre: consoles edge to edge in a regular polygon, as far out as their shelves allow, and every side carries a console except those that would cut into the cargo corridor from the door, which is the gap the ring leaves there. The ring doesn't follow the hull's segments; its shape suits the hull. In the round dome it is as many 32-column consoles as fit — **21 sides**, 4.25 m from the centre, **19 of them with modules**, about 325° of the way round. In the square box it is an **octagon** — four sides facing the walls, four cutting the corners — with sides as wide as fit: those facing a wall end up with the minimum shelf, and the corner ones always have more room (an octagon in a square leaves a triangle in each corner). The large box's sides are 90 columns, **7 sides** with modules, 4.35 m from the centre; the small one's are 38 columns, **5 sides** (the corridor takes the three at the back), 1.83 m from the centre.

**Modules.** Each side is a straight run of **modules**, each a type and a width in columns: the dome's sides are one 32-column module; the box's are three, the main one with a flank on each side taking the rest — 29 + 32 + 29 in the large box, cabinets; 3 + 32 + 3 in the small one, too narrow for a cabinet (under 8 columns), so the lectern carries on **blank**, with no dashboard. The middle one is the **main** module: a lectern the seat faces, and on the front side the one with the flight panel. The types:
...
| Large box interior (W × H × L) | 252 × 126 × 252 | 10.08 × 5.04 × 10.08 |
| Small box interior (W × H × L) | 126 × 126 × 126 | 5.04 × 5.04 × 5.04 |
| Box door | 48 × 56 | 1.92 × 2.24 |
| Box front window (W × H), sill 24 cells up | 80 × 48 | 3.20 × 1.92 |
| Main module / flank in the large box / in the small box (columns) | 32 / 29 / 3 | 1.28 / 1.16 / 0.12 |
| Tall cabinet / short cabinet (height) | — | 2.00 / 0.95 |

The ring's apothem follows from its sides and the columns per side: `a = columns · 0.04 / (2 · tan(180° / sides))` — 4.25 m for the dome's 21 × 32, 4.35 m for the large box's 8 × 90, 1.83 m for the small box's 8 × 38. The dome takes the most sides that fit; the box, the widest sides.
...
The cylinder's diameter equals the main door's width (2.24 m in the dome, its three door segments at the floor; 1.92 m in both boxes).
...
It comes in five versions from the debug panel — **Spawn → Nave domo** or **Nave cúbica**, each **(1 jugador)** or **(4 jugadores)**, and **Nave cúbica chica (3 jugadores)**: one seat, the pilot's at the front console; four — three more, at consoles −4, +4 and +7 in the dome and −2, +2 and +3 in the large box (their consoles still empty; not symmetric on purpose); or, in the small box, three — the pilot's and the two facing the side walls, −2 and +2. Each seat faces its console at the pilot's distance. Several ships can coexist — each new one lands clear of the others — and **Spawn → Limpiar spawns** removes them all. Spawned locally: `NetSpawner` attaches network sync and a grab handle to every rigid body it spawns, which would make the ship grabbable. **Not networked yet.**
