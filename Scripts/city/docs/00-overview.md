...
| [technical/character-appearance-system.md](technical/character-appearance-system.md) | **Map.** Every knob that makes one character look different from another: the three mechanisms (bone length / mesh shape / placement-that-follows), the catalogue of parameters, why "if other things must move with it, make it a bone", how the face planes and accessories are positioned without code, and a ranked read on where the system gets expensive. |
